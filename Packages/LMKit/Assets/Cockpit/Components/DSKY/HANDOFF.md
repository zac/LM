...
The coordinator requested publication of only `cockpit/dsky` and its required LFS objects to `origin` (`https://github.com/zac/LM.git`). An initial upload was blocked by automatic approval review. Zac subsequently gave direct approval in this task to push the component branch and its required Git LFS assets, without merging. Publication proceeds under that explicit authorization. No merge, application changes, extraction, migration or history rewrite is part of this delivery. All changes are confined to this component directory. The exact locally prepared branch SHA is returned in the delivery message; asset bytes are pinned by the component commit and hashes above.
