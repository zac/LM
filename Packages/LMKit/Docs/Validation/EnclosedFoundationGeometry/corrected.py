...
backing_hidden=[]
for ob in groups['CommanderPanels']:
 if ob.name.startswith(('Panel_1_Backing_', 'Panel_4_Backing_')):
  hide(ob,'coordinator backing correction comparison');backing_hidden.append(ob.name)
(O/'corrected-backing-hidden.json').write_text(json.dumps(sorted(backing_hidden),indent=2))
...
render('corrected-cdr-closed',(-.5588,1.65,.20),(-.38,1.4,-.9),18)
render('corrected-front-interior-cutaway',(0,1.65,2.8),(0,1.25,-.8),26,lambda c,o:c.z>.55)
