...
public struct LunarExplorerView: View {
    let session: LunarExplorerSession

    public init(session: LunarExplorerSession) {
        ...
        .task {
            ...
            do {
                ...
                if arguments.contains("--lunar-explorer-transition-probe") {
                    try await waitForSettledTerrain(explorer)
                    for destination in [(35.0, 2.0, 8.0), (35.0, 249.0, 700.0), (0.0, 2.0, 8.0)] as [(Double, Double, Double)] {
                        let heading = explorer.headingDegrees
                        let altitude = explorer.altitudeMeters
                        let width = explorer.metersAcross
                        let logger = Logger(subsystem: LunarMapLog.subsystem, category: "TerrainTransition")
                        ...
                        for step in 1...60 {
                            ...
                            let altitudeLog: Double = log(altitude) * (1 - weight) + log(destination.1) * weight
                            explorer.altitudeMeters = exp(altitudeLog)
                            let widthLog: Double = log(width) * (1 - weight) + log(destination.2) * weight
                            explorer.metersAcross = exp(widthLog)
