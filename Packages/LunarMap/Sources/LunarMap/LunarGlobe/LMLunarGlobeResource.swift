...
package enum LMLunarGlobeResource {
    private static let logger = Logger(
        subsystem: LunarMapLog.subsystem,
        category: "LunarGlobe"
    )

    package struct GlobeResource {
        package let entity: ModelEntity
        package let textureTier: LMTerrainManifest.Globe.TextureTier
        package let usedFallback: Bool
        package var imagery: LMLunarGlobeImagery? = nil
    }
    ...
    @MainActor
    package struct TerminatorResource {
        package let entity: ModelEntity
    ...
    @MainActor
    package static func makeEntity(
    ...
    @MainActor
    package static func makeTerminator(
        manifest: LMTerrainManifest,
        date: Date,
        frontCoordinate: LMSelenographicCoordinate? = nil,
        bundle: Bundle = LunarMap.resources
    ...
    nonisolated static func prepareTerminator(manifest: LMTerrainManifest, date: Date,
                                              bundle: Bundle = LunarMap.resources) throws -> PreparedTerminator {
    ...
    @MainActor
    package static func updateTerminator(
