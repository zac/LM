...
package struct LMLunarLandingRehearsal {
    let surface: LMTerrainContactSurface
    package private(set) var position: LMVector3D
    private(set) var velocity = LMVector3D(z: -0.5)
    package private(set) var attitude = LMQuaternion.identity
    private(set) var angularVelocity = LMVector3D.zero
    package private(set) var gear = LMLandingGearState()
    private(set) var isSettled = false
    private(set) var elapsed = 0.0

    package init(surface: LMTerrainContactSurface, north: Double, east: Double) {
    ...
    package mutating func step(seconds: Double = 1.0 / 60.0) {
    ...
    package var finished: Bool { isSettled || gear.failure != nil || elapsed >= 30 }
