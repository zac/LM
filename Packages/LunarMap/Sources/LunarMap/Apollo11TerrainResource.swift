...
public struct Apollo11TerrainHeightField: Equatable, Sendable {
    public let tile: LMTerrainManifest.Tile
    let heights: [Float]
    public let craterCatalog: LMLunarCraterCatalog?

    public var width: Int { tile.postsPerSide }
    public var height: Int { tile.postsPerSide }
    public var spacingMeters: Double { tile.postSpacingMeters }

    public func relativeElevation(eastMeters: Double, northMeters: Double) -> Float? {
    ...
    public func interpolatedSurfaceNormal(
        eastMeters: Double,
        northMeters: Double
    ) -> SIMD3<Float>? {
        let spacing = spacingMeters
...
public enum Apollo11TerrainResource {
    ...
    @usableFromInline nonisolated static let detailPipeline = LMTerrainDetailPipeline(
    ...
    public nonisolated static func loadSourceBackedHeightField(
    ...
    package struct ProgressiveTileGenerationMetrics: Equatable, Sendable {
        package let meshMilliseconds: Int
        package let detailMilliseconds: Int
        package let realizationMilliseconds: Int
        package let detailModelID: String
        package let detailCacheHit: Bool
    }

    package struct ProgressiveTileEntityBuild {
        package let entity: ModelEntity
        let mesh: LMProgressiveTerrainMeshData
        package let metrics: ProgressiveTileGenerationMetrics
    }

    @MainActor
    public static func makeProgressiveTileEntity(
    ...
    @MainActor
    package static func makeProgressiveTileEntityBuild(
        heightField: any LMTerrainHeightField,
        plan: LMTerrainTilePlan,
        activePlans: [LMTerrainTilePlan]? = nil,
        geometryReplacementPlans: [LMTerrainTilePlan]? = nil,
