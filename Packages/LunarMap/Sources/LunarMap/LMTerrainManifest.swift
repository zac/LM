...
/// the manifest itself.
public struct LMTerrainManifest: Equatable, Decodable {
    ...
    package struct Globe: Equatable, Decodable {
        struct ImageryPyramid: Equatable, Decodable {
            let file: String
            let sha256: String
        }
        let imageryPyramid: ImageryPyramid?
        package struct TextureTier: Equatable, Decodable {
            let id: String
            let file: String
            let width: Int
            let height: Int
            package let mapResolutionPixelsPerDegree: Double
            let metersPerPixel: Double
            let sha256: String
            let sourceID: String
            let detail: String
    ...
    public struct Tile: Equatable, Decodable {
        let id: String
        let postsPerSide: Int
        let postSpacingMeters: Double
        package let extentMeters: Double
    ...
    package func tile(id: String) -> Tile? {
        tiles.first { $0.id == id }
    }
    ...
    package func measuredFloorMeters(at coordinate: LMSelenographicCoordinate) -> Double? {
    ...
    package var landingOriginCoordinate: LMSelenographicCoordinate {
    ...
    package var landingLocalFrame: LMSelenographicLocalFrame {
...
public struct LMTerrainFrameAlignment: Equatable, Sendable {
    ...
    @usableFromInline static let nominalP66GuidanceTouchdown = LMVector3D(
    ...
    public let terrainReferenceTouchdown: LMVector3D
    ...
    public func terrainPosition(from guidancePosition: LMVector3D) -> LMVector3D {
        LMVector3D(
