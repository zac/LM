...
public struct LMLunarTerrainRegion: Sendable {
    public let terrain: LMLunarResolvedTerrain
    public let frame: LMSelenographicLocalFrame
    package let albedo: LMMeasuredAlbedoField
    package let sourceIDs: [String]
    package let unavailableSourceIDs: [String]
    public let measuredFloorMeters: Double
    ...
    package static func bundledSitePosition(at coordinate: LMSelenographicCoordinate,
    ...
    package static func sources(at coordinate: LMSelenographicCoordinate, manifest: LMTerrainManifest,
    ...
    public static func load(at coordinate: LMSelenographicCoordinate, store: LMLunarElevationStore,
                     offline: Bool = false, bundle: Bundle = LunarMap.resources) async throws -> Self {
        ...
        guard let source = manifest.sources.first(where: { $0.productId == "LDEM_16" }),
              let url = bundle.url(forResource: source.bundledFile, withExtension: nil, subdirectory: "Terrain") ?? bundle.url(forResource: source.bundledFile, withExtension: nil) else {
...
package struct LMLunarReflectanceField: Sendable {
    package struct Slab: Sendable {
        let data: Data
