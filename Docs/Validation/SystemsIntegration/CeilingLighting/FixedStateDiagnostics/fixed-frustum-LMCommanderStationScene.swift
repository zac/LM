...
final class LMCommanderStationScene {
    ...
    #if DEBUG
    private var lastLightingDiagnosticWrite: TimeInterval = 0
    func applyLightingDiagnostics(arguments: [String] = ProcessInfo.processInfo.arguments) {
        let shellShadows = arguments.contains("--diagnostic-shell-shadows")
        let sunOff = arguments.contains("--diagnostic-sun-off")
        guard shellShadows || sunOff || arguments.contains("--lighting-diagnostic-report") else { return }
        if shellShadows, let assembly = commanderAssembly,
           let shell = try? LMCommanderStationAssembly.path("/Cabin/Shell", in: assembly.cabin) {
            setDynamicShadowCasting(true, in: shell)
        }
        if sunOff { terrainSun?.isEnabled = false }
        if let flag = arguments.first(where: { $0.hasPrefix("--diagnostic-shadow-distance=") }),
           let distance = Float(flag.split(separator: "=").last ?? ""), let sun = terrainSun {
            let bias = arguments.first(where: { $0.hasPrefix("--diagnostic-shadow-bias=") })
                .flatMap { Float($0.split(separator: "=").last ?? "") } ?? 1
            sun.shadow = .init(shadowProjection: .automatic(maximumDistance: distance), depthBias: bias)
        }
        if arguments.contains("--diagnostic-shadow-front-cull"), let sun = terrainSun, var shadow = sun.shadow {
            shadow.cullModeOverride = .front
            sun.shadow = shadow
        }
        if let flag = arguments.first(where: { $0.hasPrefix("--diagnostic-shadow-fixed=") }),
           let scale = Float(flag.split(separator: "=").last ?? ""), let sun = terrainSun {
            let orientation = sun.orientation(relativeTo: root)
            sun.setPosition(cabinFrame.position(relativeTo: root) + orientation.act(SIMD3<Float>(0, 0, 20)), relativeTo: root)
            sun.shadow = .init(shadowProjection: .fixed(zNear: 0.1, zFar: 40, orthographicScale: scale), depthBias: 1)
        }
        let now = Date().timeIntervalSinceReferenceDate
        guard now - lastLightingDiagnosticWrite >= 1 else { return }
        lastLightingDiagnosticWrite = now
        let models = commanderAssembly.map { LMCommanderStationAssembly.descendants($0.cabin) } ?? []
        let materials: [[String: Any]] = models.filter { $0.name.contains("Crown") || $0.name.contains("Roof") }.flatMap { entity in
            (entity.components[ModelComponent.self]?.materials ?? []).compactMap { material in
                guard let pbr = material as? PhysicallyBasedMaterial else { return nil }
                return ["name": entity.name, "base_color": pbr.baseColor.tint.cgColor.components ?? [],
                    "emissive_color": pbr.emissiveColor.color.cgColor.components ?? [],
                    "emissive_intensity": pbr.emissiveIntensity, "roughness": pbr.roughness.scale,
                    "specular": pbr.specular.scale, "metallic": pbr.metallic.scale,
                    "base_texture": pbr.baseColor.texture != nil,
                    "casts_shadow": entity.components[DynamicLightShadowComponent.self]?.castsShadow ?? true] as [String: Any]
            }
        }
        let lights: [[String: Any]] = LMCommanderStationAssembly.descendants(root).compactMap { entity in
            guard let light = entity.components[DirectionalLightComponent.self] else { return nil }
            return ["name": entity.name, "enabled": entity.isEnabled, "lux": light.intensity,
                "position": [entity.position(relativeTo: root).x, entity.position(relativeTo: root).y, entity.position(relativeTo: root).z],
                "world_quaternion": [entity.orientation(relativeTo: root).imag.x, entity.orientation(relativeTo: root).imag.y,
                    entity.orientation(relativeTo: root).imag.z, entity.orientation(relativeTo: root).real],
                "shadow_projection": String(describing: (entity as? DirectionalLight)?.shadow?.shadowProjection),
                "depth_bias": (entity as? DirectionalLight)?.shadow?.depthBias ?? -1,
                "cull_mode": String(describing: (entity as? DirectionalLight)?.shadow?.cullModeOverride)]
        }
        let report: [String: Any] = ["arguments": arguments, "materials": materials, "lights": lights,
            "time": now, "altitude_m": lastVehicleState?.altitudeMeters ?? -1]
        if let data = try? JSONSerialization.data(withJSONObject: report, options: [.prettyPrinted, .sortedKeys]) {
            let target = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)[0].appendingPathComponent("lighting-diagnostic.json")
