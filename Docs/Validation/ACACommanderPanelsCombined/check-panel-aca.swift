...
@main struct Check {
 @MainActor static func main() throws {
  ...
  let scene = Entity()
  let panels = try Entity.load(contentsOf: base.appendingPathComponent("CommanderPanels/CommanderPanels.usdz"))
  ...
  let report: [String:Any] = ["method":"Native macOS RealityKit AABB separation for every CommanderPanels mesh against whole ACA bounds; 729 sampled poses at 2.75 degrees; not continuous collision or hand clearance proof", "sample_count":729,"panel_mesh_count":panelModels.count,"minimum_aabb_gap_m":gap,"closest_panel_mesh":closest,"possible_aabb_overlaps":overlaps.sorted()]
