...
## FIFO input correction after coordinator review

The coordinator identified dropped completed taps in the original cancellation chain. Reproduction uses the production queue extracted from `PoweredDescentSession`, with a controlled suspended PRO send followed by 1, 2, ENTR, PRO, PRO. The old queue cancels waiting events before dispatch. The earlier sequential pulse-helper test did not exercise this boundary.

The replacement `LMDSKYInputQueue` owns one FIFO drain worker. New completed taps append events without cancelling predecessors. PRO release finishes before any successor starts. `PoweredDescentSession.stop()` explicitly discards pending events and cancels the active pulse, which still releases PRO. If new events arrive during that release, the cancelled worker remains a barrier until a fresh worker can drain them. An already-started runtime call cannot be undone. Ordinary scene deactivation does not discard completed taps. Cancelled spatial gestures still never enqueue an event.

Script keys now use the same queue and wait for completed taps already accepted; manual input stops future script generation without discarding accepted input. This closes the parallel direct-dispatch path that could otherwise overtake PRO release.

Regressions exercise this actual production queue, rather than only the pulse helper: delayed PRO plus multiple completed normal taps and repeated PRO; teardown during a delayed PRO with delayed release plus new input; teardown before dispatch. Dependencies and instrument asset/mount scope remain unchanged. Visual DSKY/headset gates remain open.

FIFO validation: the old policy produced only `[PRO down, PRO up, PRO down, PRO up]` for the six queued events; the regression failed as expected (7 passed, 1 failed). Saved in `Docs/Validation/InstrumentQueue-reproduction.json`. The fixed queue's simulator build/test passed **10 tests, 0 failures/skips**, including all original instrument tests and the three queue regressions; saved in `Docs/Validation/InstrumentQueue-fixed.json`. Result bundle: `/tmp/lm-instrument-final/Logs/Test/Test-LM-2026.09.08_12-24-16--0700.xcresult`. A method-only filter matched zero Swift Testing cases, so both meaningful before/after runs selected the entire instrument suite. No new visual acceptance is claimed for this input-only fix.

FIFO implementation revision: `8d0e1d6fcd6c6cfa1fb98a4991ea912b2891c737`, based on previously delivered `7410e9390da7042cb9a937cf22267bf91ee54fa1`. This documentation-only follow-up records the exact validated source revision.

## Follow-up simulator visual acceptance

The debug-only observer/trace harness and repeatable capture script now provide the missing DSKY rendered-face smoke evidence. See [the visual report](Validation/InstrumentVisual/README.md), original screenshots, adjacent state samples, runtime logs and executable hash. Captures use the same pinned dependencies and production mounts; the debug observer applies an inverse viewpoint transform to the whole scene. Native Simulator camera automation did not successfully frame the DSKY, and no sidebar mirror was established.

The real P64 simulation shows live digits, V16N36 monitoring, return to V06N64 after PRO, V35 lamp test and RSET. Both imported instruments appear once and their faces are visible; FDAI ball markings change while its housing remains fixed. Small legends and dark lighting limit readability conclusions. The successful key sequence was programmatic through the production FIFO; the native pointer attempt showed hover but did not confirm a completed physical key event. No simulated pinch or physical PRO acceptance is claimed. This supersedes the earlier missing DSKY camera/face smoke gate, while gesture/headset, photometry, performance and mechanical-fit gates remain open. No instrument functional bug or production mount change was introduced by this visual pass.

Validation source is based on worker `7a0d6d51bbcc43d54253274092b1f585a75f7053`; all new runtime code is opt-in and compiled only for DEBUG. The capture script records process identity and waits for the requested live display state to avoid mixing runs or missing the brief lamp-test interval. Post-harness build/test passed **10 tests, 0 failures/skips**; the xcresult summary is saved in `Docs/Validation/InstrumentVisual/tests.json`.
