...
struct LMMoonCenteredPosition: Equatable, Sendable {
    let xMeters: Double
    let yMeters: Double
    let zMeters: Double
...
struct LMSelenographicCoordinate: Equatable, Sendable {
...
/// +north, +east, and +up.
struct LMSiteENUPosition: Equatable, Sendable {
    let northMeters: Double
    let eastMeters: Double
    let upMeters: Double

    init(northMeters: Double, eastMeters: Double, upMeters: Double) {
...
struct LMSelenographicCoordinateSystem: Equatable, Sendable {
    /// IAU Mean Earth/Polar axis rendering datum used by LOLA/SLDEM products.
    static let meanEarthPolarRadiusMeters = 1_737_400.0

    let datumRadiusMeters: Double

    init(datumRadiusMeters: Double = Self.meanEarthPolarRadiusMeters) {
...
struct LMSelenographicLocalFrame: Equatable, Sendable {
    let coordinateSystem: LMSelenographicCoordinateSystem
    let anchor: LMSelenographicCoordinate
    ...
    func localDirection(_ moonFixedDirection: SIMD3<Double>) -> SIMD3<Double> {
        SIMD3(
