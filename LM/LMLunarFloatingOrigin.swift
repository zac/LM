...
struct LMLunarFrameTransform: Equatable, Sendable {
    let rotation: simd_double3x3
    let translation: SIMD3<Double>

    init(from source: LMSelenographicLocalFrame, to destination: LMSelenographicLocalFrame) {
    ...
    func position(_ point: SIMD3<Double>) -> SIMD3<Double> {
    ...
    static func renderVector(_ neu: SIMD3<Double>) -> SIMD3<Double> {
        SIMD3(neu.x, neu.z, -neu.y)
    }

    var renderRotation: simd_double3x3 {
...
struct LMLunarFloatingOrigin: Sendable {
    ...
    private(set) var frame: LMSelenographicLocalFrame
    private(set) var generation = 0

    init(frame: LMSelenographicLocalFrame) { self.frame = frame }

    @discardableResult
    mutating func update(focus: LMMoonCenteredPosition) -> Bool {
    ...
    mutating func reanchor(at coordinate: LMSelenographicCoordinate) {
