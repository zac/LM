...
struct LMLunarPOICatalog: Decodable {
    let version: Int
    let features: [Place]
    struct Place: Decodable, Identifiable {
        ...
        let category: String
        let blurb: String
        let suggestedAltitudeMeters: Double
        let suggestedHeadingDegrees: Double
        var coordinate: LMSelenographicCoordinate { .init(latitudeDegrees: latitude, longitudeDegrees: longitude, heightMeters: height ?? 0) }
    }
    static func load(bundle: Bundle = .main) throws -> Self {
        guard let url = bundle.url(forResource: "MoonPOI", withExtension: "json") else {
...
enum LMLunarNavigation {
    static func parse(_ text: String) -> LMSelenographicCoordinate? {
    ...
    static func displayRotation(from: LMSelenographicCoordinate, to: LMSelenographicCoordinate) -> simd_quatf {
        let system = LMSelenographicCoordinateSystem()
